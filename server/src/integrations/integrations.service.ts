...
export class IntegrationsService {
  ...
  async reactiveResumeV2(userId: number, path: string): Promise<ResumeEntity> {
    try {
      ...
      const resume = this.convertReactiveResumeV2JsonToResume(jsonResume);
  ...
  convertReactiveResumeV2JsonToResume(jsonResume: any) {

    reactiveResumeV2JsonImportedSchema.parse(jsonResume);

    const timestamp = dayjs().format(FILENAME_TIMESTAMP);
    merge<Partial<Resume>, DeepPartial<Resume>>(jsonResume, {
